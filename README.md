...
| `FAST2SMS_API_KEY` / `FAST2SMS_TEMPLATE_ID` / `FAST2SMS_SENDER_ID` / `FAST2SMS_ROUTE` | SMS OTP via Fast2SMS (India). "otp" route works without DLT; "dlt" route needs TemplateID + SenderID | logs the code to stdout |
| `TWILIO_ACCOUNT_SID` / `TWILIO_AUTH_TOKEN` / `TWILIO_FROM_NUMBER` | Global SMS fallback when MSG91 + Fast2SMS unset | logs the code to stdout |
